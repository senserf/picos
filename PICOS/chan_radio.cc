...
RSSICalc::RSSICalc (word n, word m, word *wt, double *dt) {
...
word RSSICalc::calculate (double sl) {
	...
	switch (Mode) {
		...
		do {
			...
			// Interpolate and return
			return (word) ((sl - SLV [ix-1]) * FAC [ix-1]) +
					VLV [ix-1];
		} while (1);
...
PowerSetter::PowerSetter (word n, word m, word *wt, double *dt) {
...
double PowerSetter::setvalue (word w) {
...
word PowerSetter::getvalue (double v) {
	...
	switch (Mode) {
		...
		do {
			...
			// Interpolate and return
			w = (word) ((v - SLV [ix-1]) / FAC [ix-1]) + VLV [ix-1];
			break;
