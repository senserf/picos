...
static word after_pwr;		// State to enter after power up/down sequence
...
Bkf:
			...
			if (dstate & DSTATE_RXAC) {
				// Don't forget about receiving packets
				if (rx_int_enable ())
					goto DR_LOOP__;
			}
			release;
		...
		if ((dstate & DSTATE_RXAC)) {
			// This means, in particular, that the RF is on; just
			// stop the receiver
			after_pwr = DR_XMRD;
			goto DR_TOFR__;
		}
...
// Here: issue the command sequence
