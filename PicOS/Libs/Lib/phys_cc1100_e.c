...
OREvnt:
		...
		set_reg (CCxxx0_FREND0, FREND0_SET | (
			(val == NULL) ? RADIO_DEFAULT_POWER :
				((*val > 7) ? 7 : (byte) (*val))
		));
		...
		ret = (int) (get_reg (CCxxx0_FREND0) & 7);
		goto RVal;
#endif
		...
		set_reg (CCxxx0_CHANNR,
			(val == NULL) ? RADIO_DEFAULT_CHANNEL :
			(byte) (*val)
		);
