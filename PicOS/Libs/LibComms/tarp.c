...
__PRIVF (PicOSNode, word, findInRtr) (nid_t sndr, seq_t seqn, address pkt) {
	...
	while (i != rtrCache->head) {
		if ((pkt != NULL || pkt == NULL && sndr == 0 && seqn == 0) && 
			...
			diag ("dupa found at %u: %x %u %u", i, pkt, sndr, seqn);
			return i;
		}
		if (--i < 0)
			i = rtrCacheSize -1;
	}
	diag ("dupa not found %x %u %u", pkt, sndr, seqn);
	return rtrCacheSize;
}
#endif
...
__PUBLF (PicOSNode, int, tarp_rx) (address buffer, int length, int *ses) {
	...
	if (msgBuf->msg_type == 0) { // dummy ack from dst
		...
		if ((i = findInRtr (msgBuf->rcv, msgBuf->hco, NULL)) <
			...
			rtrCache->fecnt++;
			dbug_rtr ("dummy drop %u %u %u", msgBuf->rcv, 
					msgBuf->hco, rtrCache->fecnt);
		}
#endif
		return TCV_DSP_DROP;
	}
	...
	if ((i = findInRtr (msgBuf->snd, msgBuf->seq_no, NULL)) <
			rtrCacheSize) {
		dbug_rtr ("dropping %x-%u.%u", rtrCache->pkt[i],
...
CacheIt:
	...
	if (rtrCache->rcnt[rtrCache->head] < 2) {
		diag ("dupa %x", rtrCache->rcnt[rtrCache->head]);
		if (rtrCache->pkt[rtrCache->head])
		  tcv_drop (rtrCache->pkt[rtrCache->head]);
		diag ("jas %x", rtrCache->rcnt[rtrCache->head]);
		tcvp_hook (buffer, &rtrCache->pkt[rtrCache->head]);
		diag ("malgosie %x %x", buffer, rtrCache->pkt[rtrCache->head]);
