...
Boolean NTable::getCoords (Long N, double &X, double &Y) {

// This one is a method of NTable (neighbor table). It locates the specified
// neighbor and returns its coordinates, if found.
...
#if TRACE_XEVENTS
	trace ("RCV DATA: %1d <- %1d", TheNode->getId (), p->DCFP_S);
#endif
...
#if TRACE_XEVENTS
	trace ("SUC DATA: %1d <- %1d", TheNode->getId (), p->DCFP_S);
#endif
...
#if TRACE_XEVENTS
	trace ("FAI DATA: %1d <- %1d", TheNode->getId (), p->DCFP_S);
#endif
