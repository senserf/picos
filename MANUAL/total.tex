...
This is beause the operation of entering the visualization mode
...
has been entered.
Note, however, that the visualization mode can be existed and entered
...
\begin{verbatim}
        int def (BIG a);
\end{verbatim}
returning 1 when {\tt a} is defined (or definite) and 0 otherwise, and
\begin{verbatim}
        int undef (BIG a);
\end{verbatim}
...
This way simulation runs are replicatable, which is important from the
...
The {\tt off} srgument specifies the offset into the parent tag's 
character content.
...
succesful conversion with {\tt sxml\_ok} and/or {\tt sxml\_error} (see above).
...
\end{tabbing}}
\noindent
where {\em expr\/} must be either of type {\tt int} or a pointer to an object
belonging to a {\tt Station} subtype.
It identifies the
...
The effect is as if the {\tt create} operation was preceded by one of the
following statements:

{\tt\begin{tabbing}
12345678\=1234\=1234\=1234\=1234\=1234\=1234\=1234\=1234\kill
\>{\tt TheStation = {\em expr};}\\
\end{tabbing}}
or
{\tt\begin{tabbing}
12345678\=1234\=1234\=1234\=1234\=1234\=1234\=1234\=1234\kill
\>{\tt TheStation = idToStation ({\em expr\/});}\\
\end{tabbing}}
\noindent
depending on whether {\em expr\/} points to a {\tt Station} object
or produces an {\tt int} type value (see sections \ref{rm_to_st_cs},
\ref{rm_to_st_cu}).

Objects belonging to types {\tt SGroup}, {\tt CGroup}, {\tt Station},
{\tt Traffic}, {\tt Link}, {\tt RFChannel}, {\tt Mailbox}, {\tt Port}, and
...
presume any build-in characteristics, except for the timing of signals
...
in two or three dimesnions and expressed in {\em DU\/}s (\sect{rm_mp_ti}).
...
\noindent
This methods describes signal attenuation depending on the distance and possibly
...
\begin{quote}
\noindent\hspace{-0.35in}{\small {\bf {\tt Boolean RFC\_cut (double x, double s);}}} \hspace{0in}\vspace{0.05in}\\
\noindent
This method returns the {\em cut off\/} distance (in {\em ETU\/}s), i.e., the
...
For example, tge default variant of {\tt RFC\_add} assumes the linear
interpretation of the signals, which is internally prefered,
...
It is posible for processes of the same type to be spawned by processes
...
\noindent
{\bf Note:} It is illegal to terminate a process (either by itself or from
another process) if the process currently {\em owns\/} some dynamically
allocatable exposable objects (see \sect{rm_ds_om_oh}).
Such objects must be deallocated before the process can be terminated.
The {\tt Kernel} process (\sect{rm_pr_or}) is exempt from this rule, i.e.,
it is always legal to request {\tt Kernel} termination
(\sect{rm_pr_or}), which operation has a special meaning.
...
{\bf earliest} of those events will reactivate the process.
...
third argument of {\tt wait} is switched on by a special option
...
{\tt -p} in order for {\tt setSP} to be accessible.
...
By the {\em process environment\/} we mean a collection of global variables
...
can appear as an activity interpreter to another process.
...
One special value, represented by the constant {\tt DEATH} (--1), is used
to denote the termination of the process to which the wait request is
addressed.
...
a purely virtual manner, with time being entirely abstract and bearng no
...
{\em ITU\/}s required to transmit a single bit (\sect{rm_to_po_cr}).
...
Iny any case, also following a negative assessment by {\tt RFC\_bot},
...
the randomized adjustement described at {\tt entry}.
...
If the last argument is not specfified (or is negative), all the bits of the
...
i.e., to the total length of the packet (\sect{rm_cl_mp}) plus the
preamble length in bits multiplied by the
transceiver's transmission rate (\sect{rm_to_tr_cr}).
...
station is one of possibly multiple receipients).
\end{quote}
...
(see~\sect{rm_po_wr}).
This is accomplished by invoking the {\tt setAevMode} moethod of
...
A mailbox can be owned either by a station or by a process, the difference
being generally irrelevant, except for understanding mailbox exposures
(\sect{rm_ex}).
...
A mailbox belonging to a station cannot be destroyed.
A dynamically created mailbox belonging to a process
can be destroyed (by {\tt delete} as long as it has no outstanding
wait requests.
...
mailbox) or {\tt NONEMPTY}, it is not
...
nonempty and one pending token has been removed from the mailbox, and
{\tt NO} otherwise.
...
type ans set its capacity to something negative, e.g.,
\begin{verbatim}
        Mailbox *bm;
          ...
        bm = create Mailbox (-1);
...
the signal that process wants to await.
...
issued to a barrier mailbox, identifying the same or different event.
...
(\sect{rm_mb_fi}).
This is possible, similar to changing a fifo mailbox to a barrier
...
events are triggered on the mailbox.
...
\begin{itemize}
\item
Whenever a packet transmission is started on a tranceiver interfaced to the
...
These can be two or three numbers dependening on the dimensionality of the
noe deployment space (\sect{rm_to_tr_cr}).
\end{itemize}

The information displayed by the ``screen'' form of the exposure contains the
same data as the ``paper'' form.
Owing to the rigid structure of the diplay template (\sect{rm_ds_tp}), the
