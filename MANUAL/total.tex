...
reassessed after a minuscule distance change occuring half-way through a packet
reception.
...
One simple way to detect run time errors is to assert Boolean properties
...
of some variables at the moment:
\begin{verbatim}
...
It is also possible to confine that output to the context of one
specific station.
By default, i.e., without {\tt -t}, every {\tt trace} command is
effective.

The program can also affect the behavior of {\tt trace} dynamically, by
executing the global function {\tt settrace}, in one of the following
variants:

\begin{verbatim}
        void settrace (FLAGS opt);
        void settrace (FLAGS opt, Long s);
        void settrace (FLAGS opt, Long s, TIME begin);
        void settrace (FLAGS opt, Long s, TIME begin, TIME end);
        void settrace (FLAGS opt, Long s, double begin);
        void settrace (FLAGS opt, Long s, double begin, double end);
\end{verbatim}

\noindent
The first argument of {\tt settrace} is a collection of additive
flags that describe
...
{\tt settrace} selects some actual header components.
...
\begin{verbatim}
        settrace (TRACE_OPTION_ETIME + TRACE_OPTION_PROCESS);
\end{verbatim}
followed by:
\begin{verbatim}
        trace ("Here is the value: %1d, n);
...
component because {\tt trace}~{(0)} completely diables {\tt trace} output.

The remaining arguments of {\tt settrace} stand for:
\begin{llist}{{\tt beginn}}
\litem{\tt s}
the numerical {\tt Id} of the station to which the tracing should be
restricted, or {\tt ANY} denoting all stations (no restriction).
The restriction means that
a {\tt trace} call occurring in the context of a different station
(with {\tt TheStation} pointing to a station with a different {\tt Id})
will be ignored.
\litem{\tt begin}
the starting time.
This is the first moment of modeled time
when the {\tt trace} statements will be activated.
By default, they are activated at time 0.
The {\tt TIME} version of the argument is in {\em ITU\/}s, while the
{\tt double} one is in {\em ETU\/}s.
\litem{\tt end}
the ending time. 
This is the time when the {\tt trace} statements will be deactivated.
By default, they are never deactivated (or are deactivated at infinite time).
As for {\tt begin}, the
{\tt TIME} version of the argument is in {\em ITU\/}s, while the
{\tt double} version is in {\em ETU\/}s.
\end{llist}

\noindent
A variant of {\tt settrace} that does not cover a given argument
(e.g., {\tt end}), leaves the corresponding setting intact.
If {\tt -t} is absent from the program call line (\sect{rm_un_ru},
the defaults are:
{\tt opt~=~TRACE\_OPTION\_TIME},
{\tt s~=~ANY},
{\tt begin~=~TIME\_0},
{\tt end~=~TIME\_inf}.
Otherwise, the command will be applied to the settings inherited from
{\tt -t}, with the following caveat.

\noindent
{\bf Note:}
The interpretation of an {\em ETU\/} value (in {\tt settrace} or
within the {\tt -t} call argument) can only be meaningful
after the {\em ETU\/} has been defined by the program (\sect{rm_mp_ti}),
which normally happens in the first state of {\tt Root} (\sect{rm_pr_or}).
Consequently, if the {\tt -t} argument includes {\tt double} values
(to be interpreted as time in {\em ETU\/}), its application is postponed until
the simulator leaves the first state of the {\tt Root} process
(or ``the network has been built,'' in the sense of \sect{rm_pr_or}).
Consequently, in such circumstances, a call to {\tt settrace} executed in
the first state of {\tt Root}\footnote{Note that if that call specfies
time values in {\em ETU\/}s, it should be placed after the call to
{\tt setEtu} (or {\tt setItu}) that assigns a meaning to the {\em ETU\/}
(\sect{rm_mp_ti}).}
may be (later) overriden by {\tt -t}.
The moral is that {\tt settrace} and {\tt -t} should be mixed with 
due care.
...
invoking the global function {\tt setTrace} in
one of the following variants:

\begin{verbatim}
        void setTrace (Boolean full);
        void setTrace (Boolean full, Long s);
        void setTrace (Boolean full, Long s, TIME begin);
        void setTrace (Boolean full, Long s, TIME begin, TIME end);
        void setTrace (Boolean full, Long s, double begin);
        void setTrace (Boolean full, Long s, double begin, double end);
\end{verbatim}

\noindent
If {\tt full} is {\tt YES}, it selects the ``full'' version of state
dumps, as described below.
The remaining arguments restrict the tracing context in the same way
as for {\tt settrace}, as explained in \sect{rm_ob_pt}.
...
specifies the time interval during which the tracing should be on and,
optionally, the station to which it should be confined.
...
\>{\em start} \\
\>{\em start\/}{\tt -}{\em stop} \\
\>{\em start\/}{\tt /}{\em station} \\
\>{\em start\/}{\tt -}{\em stop\/}{\tt /}{\em station} \\
\>{\tt /}{\em station}
\end{tabbing}}
\noindent
...
undefined, i.e., equal to inifity.
No numbers (like in the last variant in the above list) means no time
restriction ({\em start\/} is zero, {\em stop\/} is infinity).

The optional station identifier (\sect{rm_to_st_cs}) appearing at the end of
the argument (preceded by a slash) restricts the tracing to the specified
station.
This effectively means that a trace message will only be printed if
{\tt TheStation->getId}~{\tt ()} equals the indicated station {\tt Id}.
...
Note that the program can redefine tracing parameters dynamically with
{\tt settrace} (see \sect{rm_ob_pt}).
\end{quote}
...
\end{verbatim}
\noindent
The {\tt +} option (in the last case) selects the so-called ``full tracing''
