...
//
	...
	*ses = desc;
	...
	if (len == ACK_LENGTH && ptype (p) == PTYPE_ACK) {
		...
		// packet
		// diag ("RCV ACK %u %u", psernum (p), NFreeHooks);
		for (i = 0; i < N_HOOKS; i++) {
			if ((ap = hooks [i]) != NULL &&
			    psernum (ap) == psernum (p)) {
				// diag ("ACK PKT FND %x AT %u", ap, i);
				// Drop the data packet as it has been
				// acknowledged
				tcv_drop (ap);
				// Note: there is no need to do this:
				// hooks [i] = NULL;
				// i.e., reset the hook, as it is done by
				// tcv_drop
	...
	// Create an ACK for it; quietly do nothing if no room for the ACK
	// packet
	if ((ap = tcvp_new (ACK_LENGTH, TCV_DSP_XMTU, desc)) != NULL) {
		// diag ("RCV ACKING %x %u", ap, psernum (p));
		ptype (ap) = PTYPE_ACK;
		psernum (ap) = psernum (p);
	...
	// The data packet is passed to the praxis
	return TCV_DSP_RCV;
}
...
HoldIt:
	...
	// diag ("XMT FIRST %d", NFreeHooks);
	for (i = 0; i < N_HOOKS; i++) {
		if (hooks [i] == NULL) {
			// Got it
			// diag ("XMT HOOKED %x AT %d", p, i);
