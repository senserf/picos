...
fsm mbeacon {
	...
	if (IS_SATGAT && (seconds() - master_ts  > SAT_TIMEADJ || 
		...
		runfsm satcmd_out ((char*)SATCMD_STATUS);

	if (IS_SATGAT && sat_mod == SATMOD_UNINIT) // just in case
		runfsm satcmd_out ((char*)SATCMD_PDC);
...
fsm audit {
	...
	(word)(seconds() - pow_ts) >= (tag_auditFreq << POW_FREQ_SHIFT))) {
		...
		if (IS_SATGAT)
			runfsm satcmd_out ((char*)SATCMD_QUEUE);
	...
	entry CS_WAIT:
		...
		if (str_cmpn (ui_ibuf, SATIN_RNG, SAT_RNGLEN) == 0) {
			if (sat_mod == SATMOD_UNINIT)
				sat_mod = SATMOD_YES;
			runfsm satcmd_out ((char*)SATCMD_GET);
			goto SatRep;
		}

		if (str_cmpn (ui_ibuf, SATHEX_MSG, SAT_MSGLEN) == 0) {
			runfsm satcmd_out ((char*)SATCMD_H2T);
			goto SatRep;
		}
...
#endif
					// let's try this:
					runfsm satcmd_out ((char*)SATCMD_BOOT);
...
idiosyncratic void oss_report_out (char * buf) {
  ...
  if (in_report_pload(buf)) {
	...
	lbuf = form (NULL, rep_str,
		...
		in_reportPload(buf, ppload.sval[3]),
		in_reportPload(buf, ppload.sval[8]),
		in_reportPload(buf, ppload.sval[4]),
		in_reportPload(buf, ppload.sval[9]),
		in_reportPload(buf, ppload.sval[5]),
		in_reportPload(buf, ppload.sval[10])
#else
                in_reportPload(buf, ppload.sval[1]),
                in_reportPload(buf, ppload.sval[2]),
                in_reportPload(buf, ppload.sval[3]),
...
plus we dont want to flood the sat link
			if (!IS_SATGAT) { 
				...
				char *tmpb =
					form (NULL, satframe_str, noee_str);
				if (runfsm oss_out (tmpb) == 0) {
					ufree (tmpb);
				}
			}
#endif
...
// FIXME clean it into a consistent oss / sat i/f
		...
		if (!IS_SATGAT) {
			...
			char *tmpb = form (NULL, satframe_str, ui_obuf);
			if (runfsm oss_out (tmpb) == 0) {
				ufree (tmpb);
			}
		}
	...
	entry RS_INIT2:
		...
		if (!IS_SATGAT) {
			...
			char *tmpb = form (NULL, satframe_str, satstart_str);
			if (runfsm oss_out (tmpb) == 0) {
				ufree (tmpb);
			}
		}
	  ...
	  switch (cmd_line[0]) {
		...
		case ' ': proceed RS_FREE; // ignore if starts with blank
			...
			if (!IS_SATGAT) {
				...
				char *tmpb =
					form (NULL, satframe_str, satstart_str);
				if (runfsm oss_out (tmpb) == 0) {
					ufree (tmpb);
				}
			}
