...
#if UART_TCV
		tcv_control (OSFD, PHYSOPT_SETRATE, &val);
#endif
...
#if UART_DRIVER || UART_TCV
	char * lbuf = get_mem (WNONE, 4 + 11 + 1);
	if (lbuf == NULL)
		return;
	lbuf[0] = '\0';
	lbuf[1] = 2 + 11;
...
void oss_alrm_out (char * buf) {
	...
	if (len > UART_INPUT_BUFFER_LENGTH - 12)
		return;
	if ((lbuf = get_mem (WNONE, 4 + 10 + 1 +len)) == NULL)
		return;
	lbuf[0] = '\0';
	lbuf[1] = 2 + 10 + len;
	lbuf[2] = CMD_MSGOUT;
...
#if UART_DRIVER || UART_TCV
	char * lbuf = get_mem (WNONE, 4 + sizeof(nid_t) +1 +1);
	if (lbuf == NULL)
		return;
	lbuf[0] = '\0';
...
void oss_dat_out (char * buf, bool acked) {
	...
	if (is_cmdmode) {
		...
		lbuf = get_mem (WNONE, 2 + in_dat(buf, len));
		if (lbuf == NULL)  
			return;  
...
void oss_io_in() {
	...
	switch (cmd_line[1] & 0x0F) {
		...
		case IO_ADC:
			...
			net_opt (PHYSOPT_RXOFF, NULL);
			w = pin_read_adc (WNONE, w, cmd_line[2] >> 4, w1);
			net_opt (PHYSOPT_RXON, NULL);
			if (w == WNONE)  /* -1 */
				cmd_ctrl.oprc = RC_ERES;
			else {
