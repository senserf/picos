...
void msg_cmd_in (word state, char * buf) {
	...
	if (in_cmd(buf, opcode) == CMD_GET && in_cmd(buf, oplen) == 1)
		cmd_line = (byte *) get_mem (state, 8);
	else
		cmd_line = (byte *) get_mem (state, in_cmd(buf, oplen) +1);
	cmd_line[0] = '\0'; 
...
bool msg_bindReq_out (char * buf, char** buf_out) {
	if (*buf_out == NULL) {
		*buf_out = get_mem (WNONE, sizeof(msgBindReqType));
		if (*buf_out == NULL)
			return NO;
	} else
...
bool msg_nhAck_out (char * buf, char** buf_out, word rssi) {
	if (*buf_out == NULL) {
		*buf_out = get_mem (WNONE, sizeof(msgNhAckType));
		if (*buf_out == NULL)
			return NO;
	} else
...
bool msg_new_out () {
	char * buf_out = get_mem (WNONE, sizeof(msgNewType));
	if (buf_out == NULL)
		return NO;
...
bool msg_nh_out () {
	char * buf_out = get_mem (WNONE, sizeof(msgNhType));
	if (buf_out == NULL)
		return NO;
...
bool msg_alrm_out (char * buf) {
	char * buf_out = get_mem (WNONE, sizeof(msgAlrmType) + buf[1] - 9 + 1);
	if (buf_out == NULL)
		return NO;
...
bool msg_io_out () {
	char * buf_out = get_mem (WNONE, sizeof(msgIoType));
	if (buf_out == NULL)
		return NO;
...
bool msg_br_out() {
	char * buf_out = get_mem (WNONE, sizeof(msgBrType));
	if (buf_out == NULL)
		 return NO;
...
word msg_dat_out () {
	char * buf_out;
	buf_out = get_mem (WNONE, sizeof(msgDatType) + cmd_ctrl.oplen);
	if (buf_out == NULL)
		return RC_EMEM;
...
bool msg_stAck_out (char * buf) {
	char * buf_out = get_mem (WNONE, sizeof(msgStAckType));
	if (buf_out == NULL)
		return NO;
...
bool msg_ioAck_out (char * buf) {
	char * buf_out = get_mem (WNONE, sizeof(msgIoAckType));
	if (buf_out == NULL)
		return NO;
...
bool msg_datAck_out (char * buf) {
	char * buf_out = get_mem (WNONE, sizeof(msgDatAckType));
	if (buf_out == NULL)
		return NO;
