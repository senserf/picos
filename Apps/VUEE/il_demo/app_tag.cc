...
thread (rcv)
	...
	entry (RC_TRY)
		...
		rcv_psize = net_rx (RC_TRY, &rcv_buf, NULL, 0);
		if (rcv_psize <= 0) {
			app_diag_t (D_SERIOUS, "net_rx failed (%d)",
				rcv_psize);
			proceed (RC_TRY);
		}
...
thread (cmd_in)
	...
	entry (CS_INIT)
		...
		if (ui_ibuf == NULL)
			ui_ibuf = get_mem_t (CS_INIT, UI_BUFLEN);
	...
	entry (CS_WAIT)
		...
		cmd_line = get_mem_t (CS_WAIT, strlen(ui_ibuf) +1);
...
thread (root)
	...
	entry (RS_INIT)
		ui_obuf = get_mem_t (RS_INIT, UI_BUFLEN);
		if (ee_open ()) {
			leds (LED_B, LED_BLINK);
			leds (LED_R, LED_ON);
