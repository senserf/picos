...
static lword eeptr, eecur, eeval;
static word eebase, pinnum;

word eecnt;

  entry (AT_START)

	ser_out (AT_START, "Fast EEPROM test in progress ... wait ...\r\n");
	pinnum = 0;
	delay (1024, AT_RESET);
	release;
  ...
  entry (AT_EFAIL)
	...
	ser_out (AT_EFAIL, "Failed to open eeprom\r\n");
	auto_stop ();
	release;

  entry (AT_EETST)

	for (eeptr = 0; eeptr < ee_size (NULL, NULL); eeptr += 256) {
		eeval = eeptr + eebase;
		if (ee_write (WNONE, eeptr, (byte*)&eeval, 4))
			proceed (AT_EEWER);
	}

	for (eeptr = 0; eeptr < ee_size (NULL, NULL); eeptr += 256) {
		if (ee_read (eeptr, (byte*)&eeval, 4))
			proceed (AT_EERER);
		if (eeval != eeptr + eebase)
			proceed (AT_EEMIS);
	}

  entry (AT_EEOK)

	ser_out (AT_EEOK, "EEPROM OK, testing radio and pins ...\r\n");
	radio_start ();
...
static void auto_stop () {
	...
	// Bring EEPROM to a decent state
	ee_close ();
}
...
thread (root)
  ...
  entry (RS_SAUTO)
	...
	ser_in (RS_SAUTO, ibuf, IBUFLEN-1);
	auto_stop ();
