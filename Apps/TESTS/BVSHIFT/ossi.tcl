oss_interface -id 0x00010002 -speed 9600 -length 82 -connect real \
...
set CMDS(battery)	"parse_cmd_battery"
...
proc parse_cmd_battery { } {

	set klist { arm interval threshold pin }
	...
	while 1 {
		...
		switch $k {
			...
			"interval" {
				set interval [parse_value "interval" 0 60]
			}

			"threshold" {
				set threshold [parse_value "threshold" 0 4095]
			}

			"pin" {
				set pin [parse_value "pin" 0 19]
			}
		}
	}

	if { $arm == "" } { set arm 0xFF }
	if { $interval == "" } { set interval 0xFF }
	if { $threshold == "" } { set threshold 0xFFFF }
	if { $pin == "" } { set pin 0xFF }

	parse_check_empty

	oss_issuecommand 0x03 [oss_setvalues [list $threshold $arm $pin \
		$interval] "battery"]
}
...
proc show_msg_battery { msg } {

	foreach { status threshold arm pin interval } \
		[oss_getvalues $msg "battery"] { }
	...
	set res "BATTERY:\n"
	append res "  Status:    $status\n"
	append res "  Armed:     $arm\n"
	append res "  Threshold: $threshold\n"
	append res "  Interval:  $interval\n"
	append res "  Pin:       $pin"
